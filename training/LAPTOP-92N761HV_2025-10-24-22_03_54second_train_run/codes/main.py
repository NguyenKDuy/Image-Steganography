...
DATA_DIR = r'E:\Downloads\imagenet-tiny'
...
def main():
    ...
    if opt.test == '':
        ...
        if opt.Hnet != '':
            checkpointH = torch.load(opt.Hnet)
            optimizerH.load_state_dict(checkpointH['optimizer_state_dict'])
            if 'scheduler_state_dict' in checkpointH and checkpointH['scheduler_state_dict'] is not None:
                schedulerH.load_state_dict(checkpointH['scheduler_state_dict'])
            print(f"=> Loaded Hnet checkpoint from '{opt.Hnet}' (epoch {checkpointH.get('epoch', 0)})")

        # --- Resume Rnet if specified ---
        if opt.Rnet != '':
            checkpointR = torch.load(opt.Rnet)
            optimizerR.load_state_dict(checkpointR['optimizer_state_dict'])
            if 'scheduler_state_dict' in checkpointR and checkpointR['scheduler_state_dict'] is not None:
                schedulerR.load_state_dict(checkpointR['scheduler_state_dict'])
            start_epoch = checkpointR.get('epoch', 0) + 1
            print(f"=> Loaded Rnet checkpoint from '{opt.Rnet}' (epoch {start_epoch-1})")
        ...
        for epoch in range(start_epoch, opt.niter):
            ...
            if val_sumloss < globals()["smallestLoss"]:
                ...
                torch.save({
                    ...
                    'optimizer_state_dict': optimizerH.state_dict(),
                    'scheduler_state_dict': schedulerH.state_dict() if 'schedulerH' in globals() else None,
                    'val_sumloss': val_sumloss,
                    'val_hloss': val_hloss
                }, '%s/netH_epoch_%d_sumloss=%.6f_Hloss=%.6f.pth' % (
                    opt.outckpts, epoch, val_sumloss, val_hloss))
                
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': Rnet.state_dict(),
                    'optimizer_state_dict': optimizerR.state_dict(),
                    'scheduler_state_dict': schedulerR.state_dict() if 'schedulerR' in globals() else None,
                    'val_sumloss': val_sumloss,
                    'val_rloss': val_rloss
                }, '%s/netR_epoch_%d_sumloss=%.6f_Rloss=%.6f.pth' % (
...
def validation(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    Hpsnrs = AverageMeter()  # PSNR for H-net (cover vs container)
    ...
    for i, data in enumerate(val_loader, 0):
        ...
        container_np = container_img.data.cpu().numpy()
        cover_np = cover_img.cpu().numpy()
        rev_secret_np = rev_secret_img.data.cpu().numpy()
        secret_np = secret_img.cpu().numpy()
        for j in range(this_batch_size):
            # H-net metrics
            h_psnr = psnr(cover_np[j].transpose(1,2,0), container_np[j].transpose(1,2,0), data_range=1.0)
            # cover_np[j].shape = (C,H,W), transpose thành H,W,C
            h_ssim = ssim(
